...
Parameters <- Parameters[num(initial_vacc_proportion)==0.25 & num(social_norm)==0 & num(perceived_vaccine_risk)%in%c(-0.025, 0.00625, 0.05) & num(proportion_of_nodes)==1]; # (-0.0125, 0, 0.00625
Parameters <- unique(Parameters);

quantities_to_plot <- c("phys_S", "phys_I", "phys_R", "phys_V", "soc_N", "soc_H", "soc_V");
...
for(line_index in 1:nrow(Parameters))
{
	...
	risk <- convert_if_number(this_tuple$perceived_vaccine_risk);
	...
	social_norm <- convert_if_number(this_tuple$social_norm);
