...
THIS_IS_HESITANCE <- FALSE;

rm(list=ls());

if(THIS_IS_HESITANCE){
	source("/home/b2philli/Dropbox/Processing/Hes_Parameter_Values.R");
} else {
	source("/home/b2philli/Dropbox/Processing/Parameter_Values.R", user_name));
}
...
if(THIS_IS_HESITANCE){ Parameters[network_structure=="smallworld"];}

if(THIS_IS_HESITANCE){
	Parameters <- Parameters[num(initial_vacc_proportion)==0.25 & num(social_norm)==0 & num(perceived_vaccine_risk)%in%c(-0.0125, 0, 0.00625)];
} else {
	...
	# Parameters <- Params[num(size)==10000 & num(beta)==1 & num(init_prop)==0.05 & num(norm)==0 & num(risk)%in%c(0.0312, -0.0312, 0) & num(infec)%in%c(0.2,0.8)]; #
...
for(line_index in 1:nrow(Parameters))
{
	...
	for(file_index in 1:length(data_file_list))
	{
		...
		Summary_Data <- rbind(temp[time<=max_time], Summary_Data, fill=TRUE);
  	}
